...
import { Bell, Clock, Calendar, ChevronRight, Trash2, Info, Download, FileText, Shield, User, Edit3, ExternalLink } from 'lucide-react-native';
import Animated, { FadeInDown } from 'react-native-reanimated';
import { useJournalStore } from '@/stores/journalStore';
import * as Haptics from '@/lib/haptics';
import DateTimePicker from '@react-native-community/datetimepicker';
import { exportAsText } from '@/lib/exportJournal';
...
export default function SettingsScreen() {
    ...
    if (value) {
      ...
      if (!granted) {
        Alert.alert(
          ...
          'Please enable notifications in your device settings to receive nudges.',
          [{ text: 'OK' }]
        );
        return;
      }
  ...
  const handleExportEntries = async () => {
    if (entries.length === 0) {
      Alert.alert('No Entries', 'You do not have any entries to export yet.');
      return;
    }
        ...
        >
          <Animated.View entering={FadeInDown.delay(50).springify()} className="mb-8">
            ...
            <Text style={{ fontFamily: 'DMSans_400Regular' }} className="text-stone-500 text-base mt-1">
              Customize how you clear your head
            </Text>
          </Animated.View>
          ...
          <SettingsSection title="Reminders" delay={100}>
            <SettingsRow
              ...
              label="Daily nudges"
              sublabel="Get a nudge to clear your head"
              rightElement={
                <Switch
            ...
            {reminderSettings.enabled && (
              <>
                ...
                <SettingsRow
                  ...
                  label="Gentle nudges"
                  sublabel="Softer reminders when you want them"
                  isLast
                  rightElement={
                    <Switch
            ...
            {!reminderSettings.enabled && (
              <View className="px-4 py-3">
                <Text style={{ fontFamily: 'DMSans_400Regular' }} className="text-stone-400 text-sm">
                  Enable nudges to set your preferred time and days.
                </Text>
              </View>
            )}
          </SettingsSection>
            ...
            <View className="px-4 py-4">
              <View className="flex-row items-start mb-3">
                ...
                <View className="flex-1">
                  ...
                  <Text style={{ fontFamily: 'DMSans_400Regular' }} className="text-stone-400 text-sm leading-5">
                    Your entries are stored locally on your device. Nothing is shared with anyone.
                  </Text>
                </View>
              </View>
              <View className="bg-stone-50 rounded-xl p-3 mt-2 mb-3">
                <Text style={{ fontFamily: 'DMSans_400Regular' }} className="text-stone-500 text-xs leading-4">
                  AI features send data for processing, but entries are never stored on those servers.
                </Text>
              </View>
              <Pressable
                onPress={() => {
          ...
          <SettingsSection title="About" delay={300}>
            <View className="px-4 py-4">
              ...
              <Text style={{ fontFamily: 'DMSans_400Regular' }} className="text-stone-500 text-sm leading-5 mt-2">
                Brain dumps, cleaned up. Talk or type what is stuck, clean it up, then save it or let it go.
              </Text>
            </View>
          </SettingsSection>
        </ScrollView>
